...
RSpec.describe 'Auction', type: :request do
  ...
  let(:create_offer_1_json_response) do
    ...
    { 'id' => @offer1_id, 'title' => 'A valid title', 'description' => 'A valid description', 'price' => '100.00', 'open' => true, 'bids' => [] }
  end
  ...
  let(:create_bid_params) { { bid: {offer_id: @offer1_id, buyer_name: 'A valid buyer name', password: 'supersecurepassword!', amount: '110.23' } } }
  
  let(:create_bid_json_response) do
    { "id" => Offer.find(@offer1_id).bids.first.id, "buyer_name" => 'A valid buyer name', "offer_id" =>  @offer1_id, "amount" => '110.23' }
  end
  ...
  describe 'Happy Case' do
    it 'is completely implemented' do
      ...
      # Delete bid
      delete delete_bid_path, params: { bid: { id: create_bid_json_response['id'], password: create_bid_params[:bid][:password] }}
      ...
      # Make sure bid cannot be deleted afterwards
      delete delete_bid_path, params: { bid: { id: create_bid_json_response['id'], password: create_bid_params[:bid][:password] }}
      expect(response).to have_http_status(:conflict)
      get get_offer_path, params: { id: @offer1_id }
      expect(JSON.parse(response.body)).to match(create_offer_1_json_response.merge("bids" => [create_bid_json_response], "open" => false))
    end
  end
end
